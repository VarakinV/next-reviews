...
function PaginationLink({ children, enabled, href }) {
    if (!enabled) {
        return (
            <span className="border cursor-not-allowed rounded text-slate-200 text-sm">
                {children}
            </span>
        )
    }
    return (
        <Link href={href} className="border rounded text-slate-500 text-sm hover:bg-gray-200 hover:text-slate-700">
            {children}
        </Link>
    )
}
